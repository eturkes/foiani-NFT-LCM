...
N1_data <- read_excel(file.path("..", "data", "LCM_DEPs raw data.xlsx")) # Read in N1 data.

# Merge datasets.
# ---------------
N1_data <- N1_data[N1_data$Genes %in% data$name, ] # Subset to common proteins.
data <- data[data$name %in% N1_data$Genes, ]

dup <- which(duplicated(N1_data$Genes)) # Remove duplicates in "N1_data".
N1_data <- N1_data[-dup, ]

N1_data[match(N1_data$Genes, data$name), ] # Match order of proteins.

data <- cbind(data, N1_data[ , 14:20]) # Merge
rm(N1_data) # "N1_data" object no longer needed.
data <- data[ , c(1:60, 63:ncol(data), 61:62)] # Fix order.
colnames(data) <- c( # Make naming scheme consistent. Use "Donor0" for "N1_data" samples.
  colnames(data)[1:60], "Donor0_Pos_TechRep1", "Donor0_Pos_TechRep2",
  "Donor0_Pos_TechRep3", "Donor0_Neg_TechRep1", "Donor0_Neg_TechRep2",
  "Donor0_Neg_TechRep3", "Donor0_Neg_TechRep4", colnames(data[68:69])
)
# ---------------
...
experimental_design <- data.frame(
  label = colnames(data)[6:67],
  condition = sub("^[^_]*_([^_]*).*", replacement = "\\1", colnames(data)[6:67]),
  techrep = sub(".*_", replacement = "", colnames(data)[6:67]),
  donor = sub("_.*", replacement = "", colnames(data)[6:67])
)
...
# ----------------------------------------------------
data <- make_se(data, columns = 6:67, expdesign = experimental_design)
...
```{r}
keep <- rownames(df[which(df$PC1 < 31 & df$PC1 > -25 & df$PC2 < -20 | df$PC2 > 50), ])
