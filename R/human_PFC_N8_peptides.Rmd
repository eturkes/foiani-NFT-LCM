---
title: "Human PFC N8 Peptide Analysis"
knit: (function(inputFile, encoding) {
  rmarkdown::render(
    inputFile,
    encoding = encoding,
    output_file = file.path("..", "results", "human_PFC_N8_peptides.html")
  )})
---
...
data <- read.delim(file.path("..", "data", "human_PFC_N8", "report.pr_matrix.tsv")) # Read in data.

# Use some regex magic to tidy up sample names.
# ---------------------------------------------
colnames(data) <- sub("^.*?([0-9]+_[^_]+_[0-9]+_[^_]+).*", replacement = "\\1", x = colnames(data))
colnames(data) <- c(colnames(data)[1:10], paste0("Donor", colnames(data)[11:65]))
colnames(data) <- c(
  colnames(data)[1:10], sub("(.*_){1}(\\d+)_.+", replacement = "\\1TechRep\\2", colnames(data)[11:65])
)
# ---------------------------------------------
...
# Merge gene names with peptide names.
# ------------------------------------
data$name <- paste(data$Genes, data$Precursor.Id, sep = "_")
...
experimental_design <- data.frame(
  label = colnames(data)[11:65],
  condition = sub("^[^_]*_([^_]*).*", replacement = "\\1", colnames(data)[11:65]),
  techrep = sub(".*_", replacement = "", colnames(data)[11:65]),
  donor = sub("_.*", replacement = "", colnames(data)[11:65])
)
...
# ----------------------------------------------------
data <- make_se(data, columns = 11:65, expdesign = experimental_design)
...
```{r}
keep <- rownames(df[which(df$PC2 < -25), ]) # Values towards the bottom seem to have the highest quality results.
...
data_bak <- data
tmp_design <- experimental_design[experimental_design$label %in% colnames(data), ]
design_formula <- model.matrix(~ 0 + tmp_design$condition)
corr_fit <- duplicateCorrelation(assay(data), design = design_formula, block = tmp_design$donor)

assay(data) <- removeBatchEffect(
  assay(data),
  batch = tmp_design$donor,
  design = model.matrix(
    ~ 0 + tmp_design$condition, block = tmp_design$donor, cor = corr_fit$consensus.correlation
